...
if authentication_status:
    ...
    sections = ["🏠 Home", "➕ Add Transaction", "📋 Transactions", "🧾 Expense Breakdown", "💡 Tips", "🎯 Savings Goals"]
    ...
    # =========================
    anchor("home")
    center_header("💰 SmartSave AI — Budget & Savings", 1)
    st.caption("Track expenses & income, auto-apply monthly recurring items, and set savings goals.")
    st.markdown("---")
    ...
    with st.form("transaction_form", clear_on_submit=True):
        ...
        t_category = col2.text_input("Category (e.g. Food, Transport, Salary)", value="")
        t_amount_raw = col3.text_input("Amount (e.g. 250.00)", placeholder="e.g. 250.00")

        rcol1, rcol2 = st.columns([2,2])
        make_recurring = rcol1.checkbox("Make this a recurring transaction")
        recurring_interval = rcol2.selectbox("Repeat every", ["monthly"], disabled=not make_recurring)
    ...
    if st.session_state.transactions:
        ...
        search_term = fcol1.text_input("🔍 Search by category or type", placeholder="e.g. Food or Expense")
        type_filter = fcol2.selectbox("Type filter", ["All","Expense","Income"])
        show_only_recurring = fcol3.checkbox("Only recurring")

        df_display = df.copy()
        if search_term:
            mask = df_display["category"].str.contains(search_term, case=False, na=False) | df_display["type"].str.contains(search_term, case=False, na=False)
        ...
        h1.write("Timestamp"); h2.write("Type"); h3.write("Category"); h4.write("Amount"); h5.write("Recurring"); h6.write("Actions")

        for _, row in df_display.iterrows():
            rid = row["id"]
            c1,c2,c3,c4,c5,c6 = st.columns([2,1,2,1.2,1,1])
            c1.write(row["timestamp"]); c2.write(row["type"]); c3.write(row["category"]); c4.write(f"¥{row['amount']:.2f}")
            c5.write("Yes" if row.get("recurring") else "No")
            a1,a2 = c6.columns(2)
            if a1.button("✏️", key=f"edit_{rid}"):
                st.session_state.editing_id = rid
                scroll_to("edit_block")
            if a2.button("🗑️", key=f"del_{rid}"):
        ...
        if st.session_state.editing_id:
            ...
            if record:
                with st.form("edit_form"):
                    ...
                    new_recurring = rec_col1.checkbox("Recurring monthly", value=bool(record.get("recurring")))
                    ...
                    if save_btn:
                        ...
                        if new_amount is None:
                            ...
                            record.update({
                                ...
                                "category": new_category.strip() or "Misc",
                                "timestamp": datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"),
    ...
    if st.session_state.goals:
        try:
            df_all=pd.DataFrame(st.session_state.transactions)
            cur_balance=(pd.to_numeric(df_all[df_all["type"]=="Income"]["amount"], errors="coerce").sum() - 
                         pd.to_numeric(df_all[df_all["type"]=="Expense"]["amount"], errors="coerce").sum())
        except Exception:
            cur_balance=0.0
        for g in st.session_state.goals:
            prog = min(cur_balance/g["target"],1.0) if g["target"]>0 else 0.0
            st.write(f"**{g['name']}**: ¥{cur_balance:.2f} / ¥{g['target']:.2f}")
            st.progress(prog)
    else:
